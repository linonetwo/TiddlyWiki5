...
(function() {
	...
	describe("'reduce' and 'intersection' filter prefix tests", function() {
		...
		it("should handle the :cascade filter prefix", function() {
			expect(wiki.filterTiddlers("[[Rice Pudding]] :cascade[all[shadows+tiddlers]tag[$:/tags/Filter]get[text]]").join(","))
				.toBe("It is not customary");
			expect(
				wiki.filterTiddlers("[[chocolate cake]] :cascade[all[shadows+tiddlers]tag[$:/tags/Filter]get[text]]").join(","),
			).toBe("It is customary");
			expect(
				wiki.filterTiddlers("[[Sparkling water]] :cascade[all[shadows+tiddlers]tag[$:/tags/Filter]get[text]]").join(","),
			).toBe("Just a default");
			expect(
				wiki.filterTiddlers("[[Rice Pudding]] :cascade[all[shadows+tiddlers]tag[$:/tags/SecondFilter]get[text]]").join(","),
			).toBe("It is not customary");
			expect(
				wiki.filterTiddlers("[[chocolate cake]] :cascade[all[shadows+tiddlers]tag[$:/tags/SecondFilter]get[text]]").join(
					",",
				),
			).toBe("It is customary");
			expect(
				wiki.filterTiddlers("[[Sparkling water]] :cascade[all[shadows+tiddlers]tag[$:/tags/SecondFilter]get[text]]").join(
					",",
				),
			).toBe("");
		});
		...
		it("should handle the :reduce filter prefix", function() {
			...
			expect(wiki.filterTiddlers("[tag[shopping]] :reduce[get[price]multiply{!!quantity}add<accumulator>]").join(","))
				.toBe("27.75");
			expect(
				wiki.filterTiddlers(
					"[tag[shopping]] :reduce[<index>compare:number:gt[0]then<accumulator>addsuffix[, ]addsuffix<currentTiddler>else<currentTiddler>]",
				).join(","),
			).toBe("Brownies, Chick Peas, Milk, Rice Pudding");
			// Empty input should become empty output
			expect(wiki.filterTiddlers("[tag[non-existent]] :reduce[get[price]multiply{!!quantity}add<accumulator>]").length)
				.toBe(0);
			expect(
				wiki.filterTiddlers("[tag[non-existent]] :reduce[get[price]multiply{!!quantity}add<accumulator>] :else[[0]]").join(
					",",
				),
			).toBe("0");

			expect(
				wiki.filterTiddlers("[tag[non-existent]] :reduce:11,22[get[price]multiply{!!quantity}add<accumulator>] :else[[0]]")
					.join(","),
			).toBe("0");

			expect(
				wiki.filterTiddlers("[tag[non-existent]] :reduce:11[get[price]multiply{!!quantity}add<accumulator>] :else[[0]]")
					.join(","),
			).toBe("0");
		});
		...
		it("should handle the reduce operator", function() {
			...
			expect(wiki.filterTiddlers("[tag[shopping]reduce<join-with-commas>]", anchorWidget).join(",")).toBe(
				"Brownies, Chick Peas, Milk, Rice Pudding",
			);
			// Empty input should become empty output
		...
		it("should handle the :intersection prefix", function() {
			expect(wiki.filterTiddlers("[[Sparkling water]tags[]] :intersection[[Red wine]tags[]]").join(",")).toBe(
				"drinks,textexample",
			);
		...
		it("should handle the :filter prefix and filter operator", function() {
			...
			rootWidget.setVariable("echo", "$text$", [{name: "text"}], true);
			rootWidget.setVariable(
				"larger-than-18-macro-param",
				"[subfilter<echo '[get[text]length[]compare:integer:gteq[18]]'>]",
			);
			expect(
				wiki.filterTiddlers("[tag[textexample]] :filter[get[text]length[]compare:integer:gteq[18]]", anchorWidget).join(
					",",
				),
			).toBe("Cheesecake,Chocolate Cake,Red wine");
			expect(wiki.filterTiddlers("[tag[textexample]]", anchorWidget).join(",")).toBe(
				"Cheesecake,Chocolate Cake,Red wine,Sparkling water",
			);
			expect(wiki.filterTiddlers("[tag[textexample]filter<larger-than-18>]", anchorWidget).join(",")).toBe(
				"Cheesecake,Chocolate Cake,Red wine",
			);
			expect(wiki.filterTiddlers("[tag[textexample]filter<larger-than-18-with-var>]", anchorWidget).join(",")).toBe(
				"Cheesecake,Chocolate Cake,Red wine",
			);
			// #7155
			expect(wiki.filterTiddlers("a +[filter<undefined-variable>]", anchorWidget).join(",")).toBe("a");
			expect(wiki.filterTiddlers("[tag[textexample]filter<larger-than-18-macro-param>]", anchorWidget).join(",")).toBe(
				"Cheesecake,Chocolate Cake,Red wine",
			);
		});

		it("should handle the :sort prefix", function() {
			expect(wiki.filterTiddlers("a1 a10 a2 a3 b10 b3 b1 c9 c11 c1 :sort:alphanumeric[{!!title}]").join(",")).toBe(
				"a1,a2,a3,a10,b1,b3,b10,c1,c9,c11",
			);
			expect(wiki.filterTiddlers("a1 a10 a2 a3 b10 b3 b1 c9 c11 c1 :sort:alphanumeric:reverse[{!!title}]").join(",")).toBe(
				"c11,c9,c1,b10,b3,b1,a10,a3,a2,a1",
			);
			expect(wiki.filterTiddlers("[tag[shopping]] :sort:number:[get[price]]").join(",")).toBe(
				"Milk,Chick Peas,Rice Pudding,Brownies",
			);
			expect(wiki.filterTiddlers("[tag[textexample]] :sort:number:[get[text]length[]]").join(",")).toBe(
				"Sparkling water,Chocolate Cake,Red wine,Cheesecake",
			);
			expect(wiki.filterTiddlers("[tag[textexample]] :sort:number:reverse[get[text]length[]]").join(",")).toBe(
				"Cheesecake,Red wine,Chocolate Cake,Sparkling water",
			);
			expect(wiki.filterTiddlers("[tag[notatag]] :sort:number[get[price]]").join(",")).toBe("");
			expect(wiki.filterTiddlers("[tag[cakes]] :sort:string[{!!title}]").join(",")).toBe(
				"cheesecake,Cheesecake,chocolate cake,Chocolate Cake,Persian love cake,Pound cake",
			);
			expect(wiki.filterTiddlers("[tag[cakes]] :sort:string:casesensitive[{!!title}]").join(",")).toBe(
				"Cheesecake,Chocolate Cake,Persian love cake,Pound cake,cheesecake,chocolate cake",
			);
			expect(wiki.filterTiddlers("[tag[cakes]] :sort:string:casesensitive,reverse[{!!title}]").join(",")).toBe(
				"chocolate cake,cheesecake,Pound cake,Persian love cake,Chocolate Cake,Cheesecake",
			);
		});

		it("should handle the :map prefix", function() {
			expect(wiki.filterTiddlers("[tag[shopping]] :map[get[title]]").join(",")).toBe(
				"Brownies,Chick Peas,Milk,Rice Pudding",
			);
			expect(wiki.filterTiddlers("[tag[shopping]] :map[get[description]]").join(",")).toBe(
				"A square of rich chocolate cake,a round yellow seed,,",
			);
			expect(wiki.filterTiddlers("[tag[shopping]] :map[get[description]else{!!title}]").join(",")).toBe(
				"A square of rich chocolate cake,a round yellow seed,Milk,Rice Pudding",
			);
			...
			expect(wiki.filterTiddlers("[tag[shopping]] :map:flat[tags[]]").join(",")).toBe(
				"shopping,food,shopping,food,shopping,dairy,drinks,shopping,dairy",
			);
			// Prepend the position in the list using the index and length variables
			expect(
				wiki.filterTiddlers("[tag[shopping]] :map[get[title]addprefix[-]addprefix<length>addprefix[of]addprefix<index>]")
					.join(","),
			).toBe("0of4-Brownies,1of4-Chick Peas,2of4-Milk,3of4-Rice Pudding");
		});
		...
		it("should handle macro parameters for filter run prefixes", function() {
			...
			expect(wiki.filterTiddlers('[tag[shopping]] :map[<echo "$(index)$ $(currentTiddler)$">]', anchorWidget).join(","))
				.toBe("0 Brownies,1 Chick Peas,2 Milk,3 Rice Pudding");
			// :reduce prefix
			expect(wiki.filterTiddlers("1 :reduce[subfilter<greet Tom>join[ ]]", anchorWidget).join(",")).toBe("Hello Tom");
			expect(
				wiki.filterTiddlers('[tag[shopping]] :reduce[<echo "$(accumulator)$ $(index)$ $(currentTiddler)$,">]', anchorWidget)
					.join(","),
			).toBe(" 0 Brownies, 1 Chick Peas, 2 Milk, 3 Rice Pudding,");
		});
	});
