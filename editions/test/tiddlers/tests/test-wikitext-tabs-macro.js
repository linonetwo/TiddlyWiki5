...
(function() {
	...
	describe("Tabs-macro HTML tests", function() {
		...
		wiki.addTiddler({
			title: "test-tabs-macro-horizontal",
			text:
		...
		wiki.addTiddler({
			title: "test-tabs-macro-horizontal-all",
			text:
		...
		wiki.addTiddler({
			title: "test-tabs-macro-vertical",
			text:
	...
	xit("should render tabs vertical tabs v5.2.1", function() {
		...
		*/
		...
		it("should render all 'horizontal' tabs from v5.2.2 and up with whitespace trim", function() {
			expect(wiki.renderTiddler("text/html", "test-tabs-macro-horizontal-all")).toBe(
				expectedAll.fields.text.replace(/\n/g, ""),
			);
		});

		// vertical
		it("should render 'vertical' tabs from v5.2.2 and up with whitespace trim", function() {
			expect(wiki.renderTiddler("text/html", "test-tabs-macro-vertical")).toBe(
				expectedVert.fields.text.replace(/\n/g, ""),
			);
		});
	});
})();
