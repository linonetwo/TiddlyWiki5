	...
	*/
			...
			$tw.utils.each(rects, function(rect) {
				...
				if(options.onclick) {
					domOverlay.addEventListener("click", function(event) {
						var modifierKey = event.ctrlKey && !event.shiftKey
							? "ctrl"
	...
	DynannotateWidget.prototype.applyAnnotations = function() {
		...
		if(this.hasAttribute("target")) {
			var result = textMap.findText(
				this.getAttribute("target"),
				this.getAttribute("targetPrefix"),
				this.getAttribute("targetSuffix"),
			);
			if(result) {
				this.createOverlay({
...
Selectively refreshes the widget if needed. Returns true if the widget or any of its children needed re-rendering
	*/
	DynannotateWidget.prototype.refresh = function(changedTiddlers) {
		...
		if(
			changedAttributes.selection || changedAttributes.selectionPrefix || changedAttributes.selectionSuffix ||
			changedAttributes.selectionPopup
		) {
			this.applySelectionTrackerData();
		}
		...
		var wrapperWidth = this.domWrapper.offsetWidth,
			hasResized = wrapperWidth !== this.wrapperWidth ||
				changedTiddlers["$:/state/DynaView/ViewportDimensions/ResizeCount"],
