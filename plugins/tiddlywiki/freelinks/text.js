...
Compute the internal state of the widget
	*/
	TextNodeWidget.prototype.execute = function() {
		...
		if(
			this.getVariable("tv-wikilinks", {defaultValue: "yes"}).trim() !== "no" &&
			this.getVariable("tv-freelinks", {defaultValue: "no"}).trim() === "yes" && !this.isWithinButtonOrLink()
		) {
			// Get the information about the current tiddler titles, and construct a regexp
			this.tiddlerTitleInfo = this.wiki.getGlobalCache(
				"tiddler-title-info-" + (ignoreCase ? "insensitive" : "sensitive"),
				function() {
					...
					if(match) {
						...
						childParseTree[index] = {
							...
							attributes: {
								to: {
									type: "string",
									value: ignoreCase ? this.tiddlerTitleInfo.titles[match.indexOf(match[0], 1) - 1] : match[0],
								},
	...
	TextNodeWidget.prototype.refresh = function(changedTiddlers) {
		...
		$tw.utils.each(changedTiddlers, function(change, title) {
			if(change.isDeleted) {
				...
				titlesHaveChanged = titlesHaveChanged || !self.tiddlerTitleInfo ||
