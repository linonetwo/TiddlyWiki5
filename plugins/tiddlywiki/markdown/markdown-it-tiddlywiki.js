...
(function() {
	...
	function render_footnote_ref(tokens, idx, options, env, slf) {
		...
		return '<a class="footnote-ref" href=<<qualify "##fn' + id + '">> id=<<qualify "#fnref' + refid + '">>>' + caption +
			"</a>";
	}
			...
			$tw.utils.each(twNode.attributes, function(attr, id) {
				switch (attr.type) {
					...
					case "macro":
						token.attrSet(id, {
							type: "macro",
							value: ruleinfo.rule.parser.source.substring(attr.value.start, attr.value.end),
						});
						break;
					default:
	...
	module.exports = function tiddlyWikiPlugin(markdown, options) {
		...
		if(pluginOpts.renderWikiText) {
			...
			md.inline.ruler.at(
				"html_inline",
				extendHtmlInline(md.inline.ruler.__rules__[md.inline.ruler.__find__("html_inline")].fn),
			);
