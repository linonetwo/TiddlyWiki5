...
(function() {
		...
		$tw.utils.each(elements, function(element) {
			...
			if(title) {
				...
				if(
					...
					!(elementRect.left > (viewportRect.right + viewportWidth) ||
	...
	function saveScrollPosition() {
		if(!localStorageHasFailed) {
			if(
				hasRestoredScrollPosition && $tw.wiki.getTiddlerText("$:/config/DynaView/RestoreScrollPositionAtStartup") === "yes"
			) {
				var top = findTopmostTiddler();
				if(top.element) {
					try {
						window.localStorage.setItem(
							LOCAL_STORAGE_KEY_PREFIX + window.location.pathname,
							JSON.stringify({
								title: top.title,
								offset: top.offset,
							}),
	...
	function saveViewportDimensions() {
		...
		if(document.querySelector(".tc-dynaview-request-refresh-on-resize")) {
			if(previousViewportWidth !== viewportWidth || previousViewportHeight !== viewportHeight) {
				...
				$tw.wiki.addTiddler(
					new $tw.Tiddler({title: "$:/state/DynaView/ViewportDimensions/ResizeCount", text: (count + 1) + ""}),
				);
		...
		if($tw.wiki.getTiddlerText("$:/config/DynaView/ViewportDimensions") === "yes") {
			if($tw.wiki.getTiddlerText("$:/state/DynaView/ViewportDimensions/Width") !== viewportWidth.toString()) {
				$tw.wiki.setText(
					"$:/state/DynaView/ViewportDimensions/Width",
					undefined,
					undefined,
			...
			if($tw.wiki.getTiddlerText("$:/state/DynaView/ViewportDimensions/Height") !== viewportHeight.toString()) {
				$tw.wiki.setText(
					"$:/state/DynaView/ViewportDimensions/Height",
					undefined,
					undefined,
