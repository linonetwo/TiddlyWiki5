...
(function() {
	...
	function Slicer(options) {
		...
		this.chunks.push({
			...
			text: '<div class=\'tc-table-of-contents\'><<toc-selective-expandable """' + this.baseTiddlerTitle +
				'document""">></div>',
			list: [],
			tags: [],
			role: this.role,
	...
	Slicer.prototype.getSourceText = function() {
		if(this.sourceTiddlerTitle) {
			...
			if(
				tiddler.fields.type === "text/html" || tiddler.fields.type === "text/xml" ||
				(tiddler.fields.type || "").slice(-4) === "+xml"
			) {
	...
	Slicer.prototype.onText = function(text) {
		...
		if(
			this.elementStack.some(function(e) {
