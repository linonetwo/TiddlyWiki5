...
It is the responsibility of the filesystem adaptor to update this.boot.files for new files that are created.
	*/
	FileSystemAdaptor.prototype.getTiddlerFileInfo = function(tiddler, callback) {
		...
		var title = tiddler.fields.title, newInfo, pathFilters, extFilters, fileInfo = this.boot.files[title];
			...
			$tw.utils.saveTiddlerToFile(tiddler, fileInfo, function(err, fileInfo) {
				if(err) {
					if((err.code == "EPERM" || err.code == "EACCES") && err.syscall == "open") {
						...
						$tw.syncer.logger.log(
							'Sync failed for "' + tiddler.fields.title + '" and will be retried with encoded filepath',
							encodeURIComponent(fileInfo.filepath),
						);
						return callback(err);
					} else {
						return callback(err);
