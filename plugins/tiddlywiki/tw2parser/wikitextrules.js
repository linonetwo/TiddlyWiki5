...
(function() {
	...
	textPrimitives.tiddlerForcedLinkRegExp = new RegExp(
		"(?:" + textPrimitives.titledBrackettedLink + ")|(?:" +
			textPrimitives.brackettedLink + ")|(?:" +
			textPrimitives.urlPattern + ")",
		"mg",
	);

	textPrimitives.tiddlerAnyLinkRegExp = new RegExp(
		"(" + textPrimitives.wikiLink + ")|(?:" +
			textPrimitives.titledBrackettedLink + ")|(?:" +
			textPrimitives.brackettedLink + ")|(?:" +
			textPrimitives.urlPattern + ")",
		"mg",
	...
	var enclosedTextHelper = function(w) {
		...
		if(lookaheadMatch && lookaheadMatch.index == w.matchStart) {
			...
			w.output.push({type: "element", tag: this.element, children: [{type: "text", text: lookaheadMatch[1]}]});
			w.nextMatch = lookaheadMatch.index + lookaheadMatch[0].length;
		}
	};
	...
	var rules = [
		{
			...
			handler: function(w) {
				var table = {type: "element", tag: "table", attributes: {class: {type: "string", value: "table"}}, children: []};
			...
			handler: function(w) {
				...
				switch (w.matchText) {
					...
					case "`":
						...
						if(lookaheadMatch && lookaheadMatch.index == w.matchStart) {
							w.output.push({type: "element", tag: "code", children: [{type: "text", text: lookaheadMatch[1]}]});
						}
						break;
					case "{{{":
						lookaheadRegExp = /\{\{\{((?:.|\n)*?)\}\}\}/mg;
				...
				switch (w.matchText) {
					...
					case "{{":
						...
						if(lookaheadMatch) {
							...
							e = {
								type: "element",
								tag: lookaheadMatch[2] == "\n" ? "div" : "span",
								attributes: {class: {type: "string", value: lookaheadMatch[1]}},
								children: [],
							};
							w.output.push(e);
							w.subWikifyTerm(e.children, /(\}\}\})/mg);
