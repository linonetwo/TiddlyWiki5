...
Options include:
	...
	*/
	exports.generateTiddlerFilepath = function(title, options) {
		...
		var fullPath, oldPath = (options.fileInfo) ? options.fileInfo.filepath : undefined, count = 0;
		do {
	...
	exports.saveTiddlerToFile = function(tiddler, fileInfo, callback) {
		...
		if(fileInfo.hasMetaFile) {
			...
			if(fileInfo.type === "application/x-tiddler") {
				...
				fs.writeFile(
					fileInfo.filepath,
					JSON.stringify([tiddler.getFieldStrings({exclude: ["bag"]})], null, $tw.config.preferences.jsonSpaces),
					"utf8",
					function(err) {
						if(err) {
							return callback(err);
				...
				if(err) {
					if((err.code == "EPERM" || err.code == "EACCES") && err.syscall == "unlink") {
						...
						$tw.syncer.displayError(
							'Server desynchronized. Error cleaning up previous file for tiddler: "' + title + '"',
							err,
						);
