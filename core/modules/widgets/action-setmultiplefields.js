...
Refresh the widget by ensuring our attributes are up to date
	*/
	SetMultipleFieldsWidget.prototype.refresh = function(changedTiddlers) {
		...
		if(
			changedAttributes["$tiddler"] || changedAttributes["$fields"] || changedAttributes["$indexes"] ||
			changedAttributes["$values"] || changedAttributes["$timestamp"]
		) {
			this.refreshSelf();
			return true;
		}
	...
	SetMultipleFieldsWidget.prototype.invokeAction = function(triggeringWidget, event) {
		...
		if(this.actionFields) {
			...
			this.wiki.addTiddler(
				new $tw.Tiddler(creationFields, tiddler, {title: this.actionTiddler}, modificationFields, additions),
			);
