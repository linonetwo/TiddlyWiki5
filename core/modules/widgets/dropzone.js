				...
				for(var t = 0; t < dataTransfer.items.length; t++) {
					...
					if(item.kind === "string") {
						item.getAsString(function(str) {
							var tiddlerFields = self.wiki.deserializeTiddlers(null, str, {title: fallbackTitle}, {
								deserializer: self.dropzoneDeserializer,
							});
		...
		var getItem = function(type) {
			...
			return function(str) {
				...
				if(self.dropzoneDeserializer) {
					tiddlerFields = self.wiki.deserializeTiddlers(null, str, {title: self.wiki.generateNewTitle("Untitled " + type)}, {
						deserializer: self.dropzoneDeserializer,
					});
					if(tiddlerFields && tiddlerFields.length) {
						readFileCallback(tiddlerFields);
		...
		if(["TEXTAREA", "INPUT"].indexOf(event.target.tagName) == -1 && !event.target.isContentEditable && !event.twEditor) {
			...
			for(var t = 0; t < items.length; t++) {
				...
				if(item.kind === "file") {
					...
					item.kind === "string" && !["text/html", "text/plain", "Text"].includes(item.type) &&
					$tw.utils.itemHasValidDataType(item)
				) {
					// Try to import the various data types we understand
					var fallbackTitle = self.wiki.generateNewTitle("Untitled");
					// Use the deserializer specified if any
