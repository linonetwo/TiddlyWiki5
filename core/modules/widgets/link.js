	...
	LinkWidget.prototype.renderLink = function(parent, nextSibling) {
		...
		if(wikilinkTransformFilter) {
			...
			wikiLinkText = $tw.utils.replaceString(
				wikiLinkTemplate,
				"$uri_encoded$",
				$tw.utils.encodeURIComponentExtended(this.to),
			);
			wikiLinkText = $tw.utils.replaceString(
				wikiLinkText,
				"$uri_doubleencoded$",
				$tw.utils.encodeURIComponentExtended($tw.utils.encodeURIComponentExtended(this.to)),
			);
		}
		// Override with the value of tv-get-export-link if defined
		wikiLinkText = this.getVariable("tv-get-export-link", {
			params: [{name: "to", value: this.to}],
...
Selectively refreshes the widget if needed. Returns true if the widget or any of its children needed re-rendering
	*/
	LinkWidget.prototype.refresh = function(changedTiddlers) {
		...
		if(
			changedAttributes.to || changedTiddlers[this.to] || changedAttributes["aria-label"] || changedAttributes.tooltip ||
