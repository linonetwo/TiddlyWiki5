		...
		if(template) {
			...
			if(this.parseTreeNode.children && this.parseTreeNode.children.length > 0) {
				...
				templateTree = [{
					...
					children: [{
						type: "link",
						attributes: {to: {type: "string", value: title}},
						children: [
...
Selectively refreshes the widget if needed. Returns true if the widget or any of its children needed re-rendering
	*/
	ListWidget.prototype.refresh = function(changedTiddlers) {
		...
		if(
			changedAttributes.filter || changedAttributes.variable || changedAttributes.counter || changedAttributes.template ||
			changedAttributes.editTemplate || changedAttributes.emptyMessage || changedAttributes.storyview ||
...
Handle any changes to the history list
	*/
	ListWidget.prototype.handleHistoryChanges = function() {
		...
		var entry = 0;
		while(
			entry < newHistory.length && entry < this.history.length && newHistory[entry].title === this.history[entry].title
		) {
			entry++;
		}
