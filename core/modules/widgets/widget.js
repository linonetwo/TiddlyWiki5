		...
		if(variable) {
			...
			if(variable.isMacroDefinition) {
				...
				$tw.utils.each(params, function(param) {
					value = $tw.utils.replaceString(
						value,
						new RegExp("\\$" + $tw.utils.escapeRegExp(param.name) + "\\$", "mg"),
						param.value,
					);
				});
	...
	Widget.prototype.makeChildWidget = function(parseTreeNode, options) {
		...
		if(this.variables[variableDefinitionName]) {
			...
			if(!parseTreeNode.isNotRemappable && isOverrideable()) {
				...
				if(
					variableInfo && variableInfo.srcVariable && variableInfo.srcVariable.value &&
					variableInfo.srcVariable.isWidgetDefinition
				) {
