	...
	*/
	exports["text/html"] = function(text, fields) {
		...
		if(storeAreaMatch) {
			...
			results.push.apply(
				results,
				deserializeStoreArea(text, storeAreaMarkerRegExp.lastIndex, !!storeAreaMatch[1], fields),
			);
		}
		...
		while(newStoreAreaMatch) {
			results.push.apply(
				results,
				deserializeNewStoreArea(text, newStoreAreaMarkerRegExp.lastIndex, newStoreAreaMatch[1], fields),
			);
			newStoreAreaMatch = newStoreAreaMarkerRegExp.exec(text);
		}
