...
(function() {
			...
			if(this.editIndex) {
				...
				update = function(value) {
					...
					self.wiki.addTiddler(
						new $tw.Tiddler(self.wiki.getCreationFields(), tiddler, updateFields, self.wiki.getModificationFields()),
					);
				};
			}
			if(this.editType) {
	...
	*/
		EditTextWidget.prototype.refresh = function(changedTiddlers) {
			...
			if(
				changedAttributes.tiddler || changedAttributes.field || changedAttributes.index || changedAttributes["default"] ||
				changedAttributes["class"] || changedAttributes.placeholder || changedAttributes.size ||
				changedAttributes.autoHeight || changedAttributes.minHeight || changedAttributes.focusPopup ||
				changedAttributes.rows || changedAttributes.tabindex || changedAttributes.cancelPopups ||
				changedAttributes.inputActions || changedAttributes.refreshTitle || changedAttributes.autocomplete ||
				changedTiddlers[HEIGHT_MODE_TITLE] || changedTiddlers[ENABLE_TOOLBAR_TITLE] || changedTiddlers["$:/palette"] ||
				changedAttributes.disabled || changedAttributes.fileDrop
			) {
				this.refreshSelf();
				return true;
	...
	*/
		EditTextWidget.prototype.propogateKeydownEvent = function(event) {
			var newEvent = this.cloneEvent(event, [
				"keyCode",
				"code",
				"which",
				"key",
		...
		EditTextWidget.prototype.cloneEvent = function(event, propertiesToCopy) {
			var propertiesToCopy = propertiesToCopy || [],
				newEvent = this.document.createEventObject
