...
Like addTiddler() except it will silently reject any plugin tiddlers that are older than the currently loaded version. Returns true if the tiddler was imported
	*/
	exports.importTiddler = function(tiddler) {
		...
		if(
			tiddler && tiddler.hasField("plugin-type") && tiddler.hasField("version") && existingTiddler &&
			existingTiddler.hasField("plugin-type") && existingTiddler.hasField("version")
		) {
	...
	*/
	exports.renderTiddler = function(outputType, title, options) {
		...
		return outputType === "text/html"
			? container.innerHTML
			: (outputType === "text/plain-formatted" ? container.formattedTextContent : container.textContent);
	};
...
Options available:
	...
	*/
	exports.search = function(text, options) {
		...
		var terms, searchTermsRegExps, flags = options.caseSensitive ? "" : "i", anchor = options.anchored ? "^" : "";
		if(options.literal) {
			if(text.length === 0) {
			...
			if(terms.length === 1 && terms[0] === "") {
				...
				for(t = 0; t < terms.length; t++) {
					regExpStr += (t === 0)
						? anchor + $tw.utils.escapeRegExp(terms[t])
						: "|" + anchor + $tw.utils.escapeRegExp(terms[t]);
				}
		...
		var searchTiddler = function(title) {
			...
			var contentTypeInfo = $tw.config.contentTypeInfo[tiddler.fields.type] ||
					$tw.config.contentTypeInfo["text/vnd.tiddlywiki"],
				searchFields;
...
historyTitle: title of history tiddler (defaults to $:/HistoryList)
	*/
	exports.addToHistory = function(title, fromPageRect, historyTitle) {
		...
		console.log(
			"$tw.wiki.addToHistory() is deprecated since V5.1.23! Use the this.story.addToHistory() from the story-object!",
		);
	};
