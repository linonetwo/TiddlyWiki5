...
options: variables - optional hashmap of variables to set (a misnomer - they are really constant parameters)
	...
	*/
	function Server(options) {
		...
		this.servername = $tw.utils.transliterateToSafeASCII(
			this.get("server-name") || this.wiki.getTiddlerText("$:/SiteTitle") || "TiddlyWiki5",
		);
		this.boot.origin = this.get("origin")
			? this.get("origin")
			: this.protocol + "://" + this.get("host") + ":" + this.get("port");
	...
	*/
	Server.prototype.isAuthorized = function(authorizationType, username) {
		...
		return principals.indexOf("(anon)") !== -1 ||
			(username && (principals.indexOf("(authenticated)") !== -1 || principals.indexOf(username) !== -1));
	};
	...
	Server.prototype.requestHandler = function(request, response, options) {
		...
		if(
			!this.csrfDisable && state.authorizationType === "writers" && request.headers["x-requested-with"] !== "TiddlyWiki"
		) {
		...
		if(!this.isAuthorized(state.authorizationType, state.authenticatedUsername)) {
			response.writeHead(
				401,
				"'" + state.authenticatedUsername + "' is not authorized to access '" + this.servername + "'",
			);
			response.end();
			return;
		}
		...
		if(route.bodyFormat === "stream" || request.method === "GET" || request.method === "HEAD") {
			...
			response.writeHead(
				400,
				"Invalid bodyFormat " + route.bodyFormat + " in route " + route.method + " " + route.path.source,
			);
			response.end();
		}
	};
	...
	Server.prototype.listen = function(port, host, prefix) {
		...
		server.on("listening", function() {
			var address = server.address(),
				url = self.protocol + "://" + (address.family === "IPv6" ? "[" + address.address + "]" : address.address) + ":" +
					address.port + prefix;
