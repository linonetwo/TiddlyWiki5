...
Display an error and exit
	*/
	$tw.utils.error = function(err) {
		...
		var errHeading = $tw.language == undefined
				? "Internal JavaScript Error"
				: $tw.language.getString("InternalJavaScriptError/Title"),
			promptMsg = $tw.language == undefined
		...
		if($tw.browser && !$tw.node) {
			...
			var dm = $tw.utils.domMaker,
				...
				button = dm("div", {
					children: [
						dm("button", {text: ($tw.language == undefined ? "close" : $tw.language.getString("Buttons/Close/Caption"))}),
					],
...
Convert "&amp;" to &, "&nbsp;" to nbsp, "&lt;" to <, "&gt;" to > and "&quot;" to "
	*/
	$tw.utils.htmlDecode = function(s) {
		return s.toString().replace(/&lt;/mg, "<").replace(/&nbsp;/mg, "\xA0").replace(/&gt;/mg, ">").replace(/&quot;/mg, '"')
			.replace(/&amp;/mg, "&");
	};
...
Parse a semantic version string into its constituent parts -- see https://semver.org
	*/
	$tw.utils.parseVersion = function(version) {
		var match =
			/^v?((\d+)\.(\d+)\.(\d+))(?:-([\dA-Za-z\-]+(?:\.[\dA-Za-z\-]+)*))?(?:\+([\dA-Za-z\-]+(?:\.[\dA-Za-z\-]+)*))?$/.exec(
				version,
			);
		if(match) {
			return {
	...
	*/
	$tw.Wiki = function(options) {
		...
		this.eachTiddlerPlusShadows = function(callback) {
			var index, titlesLength, title, titles = getTiddlerTitles();
			for(index = 0, titlesLength = titles.length; index < titlesLength; index++) {
				title = titles[index];
				callback(tiddlers[title], title);
			}
		...
		this.eachShadowPlusTiddlers = function(callback) {
			var index, titlesLength, title, titles = getShadowTiddlerTitles();
			for(index = 0, titlesLength = titles.length; index < titlesLength; index++) {
				title = titles[index];
				if(tiddlers[title]) {
					callback(tiddlers[title], title);
		...
		this.registerPluginTiddlers = function(pluginType, titles) {
			var self = this,
				...
				checkTiddler = function(tiddler, title) {
					if(
						tiddler && tiddler.fields.type === "application/json" && tiddler.fields["plugin-type"] &&
		...
		this.unregisterPluginTiddlers = function(pluginType, titles) {
			...
			for(var t = pluginTiddlers.length - 1; t >= 0; t--) {
				...
				if(
					tiddler.fields["plugin-type"] && (!pluginType || tiddler.fields["plugin-type"] === pluginType) &&
	...
	$tw.Wiki.prototype.clearCache =
		...
		$tw.Wiki.prototype.enqueueTiddlerEvent =
			function() {};
				...
				switch(tiddler.fields.type) {
					...
					case "application/json":
						$tw.modules.define(
							...
							$tw.utils.parseJSONSafe(tiddler.fields.text),
						);
						break;
...
metadata files.
		*/
		$tw.boot.excludeRegExp =
			/^\.DS_Store$|^.*\.meta$|^\..*\.swp$|^\._.*$|^\.git$|^\.github$|^\.vscode$|^\.hg$|^\.lock-wscript$|^\.svn$|^\.wafpickle-.*$|^CVS$|^npm-debug\.log$/;
		...
		$tw.loadTiddlersFromSpecification = function(filepath, excludeRegExp) {
			...
			var processFile = function(filename, isTiddlerFile, fields, isEditableFile, rootPath) {
				...
				if(isEditableFile) {
					tiddlers.push({
						filepath: pathname,
						hasMetaFile: !!metadata && !isTiddlerFile,
						isEditableFile: true,
						tiddlers: fileTiddlers,
					});
				} else {
				...
				if(!options.readOnly && tiddlerFile.filepath) {
					$tw.utils.each(tiddlerFile.tiddlers, function(tiddler) {
						$tw.boot.files[tiddler.title] = {
							...
							isEditableFile: config["retain-original-tiddler-path"] || tiddlerFile.isEditableFile ||
								tiddlerFile.filepath.indexOf($tw.boot.wikiTiddlersPath) !== 0,
						};
					});
				}
		...
		if(name) {
			...
			for(t = 0; t < remaining.length; t++) {
				...
				if(task.before && task.before.indexOf(name) !== -1) {
					if(
						$tw.boot.doesTaskMatchPlatform(task) && (!task.name || $tw.boot.disabledStartupModules.indexOf(task.name) === -1)
					) {
						return false;
					}
				}
